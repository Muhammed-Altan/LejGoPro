...
async function bookNow() {
	...
	try {
		// For now, if no productId set, attempt booking with a default id (e.g., 1)
		const productId = store.productId ?? 1;
		const res = await fetch('http://localhost:3001/bookings/by-product', {
			method: 'POST',
